...
func (s *IntegrationTestSuite) TestQueryGRPC() {
	...
	testCases := []struct {
		...
		{
			"gRPC request params",
			fmt.Sprintf("%s/mainchain/mint/v1/params", baseURL),
			map[string]string{},
		...
		},
		{
			...
			fmt.Sprintf("%s/mainchain/mint/v1/inflation", baseURL),
			map[string]string{},
		...
		},
		{
			...
			fmt.Sprintf("%s/mainchain/mint/v1/annual_provisions", baseURL),
			map[string]string{
