...
func (wfd CorrectWrkChainFeeDecorator) AnteHandle(ctx sdk.Context, tx sdk.Tx, simulate bool, next sdk.AnteHandler) (sdk.Context, error) {
	...
	// Check the fee amount sent in the Tx. Since WrkChains have set fees that are not based
	// on gas/gas prices, we need to check the Tx carries exactly the fees the WrkChain
	...
	// every WrkChain Tx. Widening this condition is a state-machine change: it belongs in a
	...
	err := feecheck.CheckFeePayerHasFunds(ctx, wfd.bankKeeper, wfd.accKeeper, wfd.entKeeper,
		wfd.wrkchainKeeper.GetParamDenom(ctx), feeTx)
	if err != nil {
		return ctx, err
	}
